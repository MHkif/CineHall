...
class Admin extends Controller
{
    ...
    public function register($ref)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($data['email_err']) && empty($data['username_err']) && empty($data['password_err']) && empty($data['confirm_password_err']) && empty($data['avatar_err'])) {
                ...
                $SignUpUser =  $this->adminModel->register($data);
    ...
    public function login()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($this->adminModel->findUserByRef($data['ref'])) {
                // die('User Found');
            ...
            if (empty($data['ref_err']) && empty($data['password_err'])) {
                ...
                $loggedInUser = $this->adminModel->login($data['ref'], $data['password']);
    ...
    public function createUserSession($model)
    {
        ...
        // echo '</pre>';
        // exit;
        $_SESSION[$this->user . '_id'] = $model->id;
        $_SESSION[$this->user . '_email'] = $model->email;
        $_SESSION[$this->user . '_name'] = $model->name;
        redirect("$this->user/dashboard");
    }
